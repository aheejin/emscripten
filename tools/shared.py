...
class Building(object):
  ...
  def js_optimizer_no_asmjs(filename, passes, return_output=False, extra_info=None):
    ...
    if not return_output:
      ...
      mylog.log_cmd(NODE_JS + [js_optimizer.JS_OPTIMIZER, filename] + passes)
  ...
  def eval_ctors(js_file, binary_file, binaryen_bin='', debug_info=False):
    mylog.log_cmd([PYTHON, path_from_root('tools', 'ctor_evaller.py'), js_file, binary_file, str(Settings.TOTAL_MEMORY), str(Settings.TOTAL_STACK), str(Settings.GLOBAL_BASE), binaryen_bin, str(int(debug_info))])
